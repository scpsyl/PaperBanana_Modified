...
class VisualizerAgent(BaseAgent):
    ...
    async def process(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for desc_key in desc_keys_to_process:
            ...
            image_provider = generation_utils._infer_provider(self.model_name, is_image=True)

            if cfg["use_image_generation"] and image_provider == "gemini":
                aspect_ratio = "1:1"
            ...
            if cfg["use_image_generation"]:
                if image_provider == "gemini":
                ...
                elif image_provider == "openai" or "gpt-image" in self.model_name:
                    image_config = {
                        "size": "1536x1024",
                        "quality": "high",
                        "background": "opaque",
                    ...
                    raise ValueError(f"Unsupported image model: {self.model_name}. Use a model name containing 'gemini' or 'gpt-image', or set provider_override.image_model_provider.")
            else:
                # Text/code generation: use unified dispatch (supports all providers)
                response_list = await generation_utils.call_text_model_with_retry_async(
                    model_name=self.model_name,
                    contents=content_list,
                    system_prompt=gen_config_args.get("system_instruction", self.system_prompt),
                    temperature=gen_config_args.get("temperature", self.exp_config.temperature),
                    candidate_count=gen_config_args.get("candidate_count", 1),
                    max_output_tokens=gen_config_args.get("max_output_tokens", cfg["max_output_tokens"]),
                    max_attempts=5,
                    retry_delay=30,
                )
