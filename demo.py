...
def run_async(coro):
    """Run an async coroutine safely in Streamlit (avoids 'Event loop is closed')."""
...
VTRACER_DIR = Path(__file__).parent / "tools" / "vtracer_converter"
VTRACER_PYTHON = VTRACER_DIR / "venv" / "bin" / "python"
VTRACER_SCRIPT = VTRACER_DIR / "convert.py"


def convert_png_to_svg_via_vtracer(png_bytes: bytes, preset: str = "diagram") -> tuple:
    """
    Convert PNG image bytes to SVG using vtracer in its isolated venv.
    Returns (svg_string, message). No LLM calls, no token cost.
    """
    import subprocess, tempfile

    if not VTRACER_PYTHON.exists():
        return None, f"vtracer venv not found at {VTRACER_PYTHON}. Run: python3 -m venv tools/vtracer_converter/venv && tools/vtracer_converter/venv/bin/pip install vtracer Pillow"
    ...
    try:
        result = subprocess.run(
            [str(VTRACER_PYTHON), str(VTRACER_SCRIPT), input_path, output_path, "--preset", preset],
            capture_output=True, text=True, timeout=120,
        )

        if result.returncode != 0:
            return None, f"vtracer error: {result.stderr.strip()}"
        ...
        return None, "vtracer conversion timed out (>120s)"
    except Exception as e:
        return None, str(e)
    finally:
...
def display_candidate_result(result, candidate_id, exp_mode):
    ...
    if final_image_key and final_image_key in result:
        ...
        if img:
            ...
            with dl_col2:
                svg_session_key = f"svg_candidate_{candidate_id}"
                if svg_session_key in st.session_state:
                    st.download_button(
                        label="⬇️ SVG",
                        data=st.session_state[svg_session_key],
                        file_name=f"candidate_{candidate_id}.svg",
                        mime="image/svg+xml",
                        key=f"download_svg_{candidate_id}",
                        use_container_width=True
                    )
                else:
                    if st.button("🔄 Export SVG", key=f"gen_svg_{candidate_id}", use_container_width=True):
                        with st.spinner("Converting to SVG via vtracer (no API cost)..."):
                            png_buf = BytesIO()
                            img.save(png_buf, format="PNG")
                            svg_code, msg = convert_png_to_svg_via_vtracer(
                                png_buf.getvalue(), preset="diagram"
                            )
                            if svg_code:
                                st.session_state[svg_session_key] = svg_code
                                st.rerun()
                            else:
                                st.error(f"SVG conversion failed: {msg}")
        else:
    ...
    with tab1:
        ...
        with st.sidebar:
            ...
            retrieval_setting = st.selectbox(
                ...
                help="none=no retrieval (cheapest), auto=automatic selection, manual=use specified references, random=random selection"
            )
            ...
            num_candidates = st.number_input(
                ...
                help="How many parallel candidates to generate. Each candidate costs ~5-9 API calls. Start small!"
            )
            ...
            max_critic_rounds = st.number_input(
                ...
                help="Each round adds 2 API calls (Critic+Visualizer) per candidate. 0=no critic, 1=good balance"
            )
...
The framework extends to statistical plots by adjusting the Visualizer and Critic agents. For numerical precision, the Visualizer converts the description $P_t$ into executable Python Matplotlib code: $I_t = \text{VLM}_{\text{code}}(P_t)$. The Critic evaluates the rendered plot and generates a refined description $P_{t+1}$ addressing inaccuracies or imperfections: $P_{t+1} = \text{VLM}_{\text{critic}}(I_t, S, C, P_t)$. The same $T=3$ round iterative refinement process applies. While we prioritize this code-based approach for accuracy, we also explore direct image generation in Section \ref{sec:discussion}. See Appendix \ref{app_sec:plot_agent_prompt} for adjusted prompts."""
        ...
        base_calls = 3 if exp_mode == "demo_planner_critic" else 4  # retriever+planner+visualizer (+stylist)
        critic_calls = max_critic_rounds * 2  # each round = critic + visualizer
        total_api_calls = num_candidates * (base_calls + critic_calls)
        cost_color = "🟢" if total_api_calls <= 15 else ("🟡" if total_api_calls <= 40 else "🔴")
        st.warning(
            f"{cost_color} **Estimated API calls: {total_api_calls}** "
            f"({num_candidates} candidates × {base_calls + critic_calls} calls each). "
            f"Tip: start with 1-3 candidates and 0-1 critic rounds to save tokens."
        )
